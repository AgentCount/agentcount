...
| `GET /api/runs/{id}/delta` | What changed between this run and the previous finished run on the same chain, exactly as the `delta` binary stored it: `newly_registered`, `disappeared`, `newly_resolving`, `stopped_resolving` (both series exclude transitions touching `refused` or `error`; `rung2_declined` and `rung2_errored` total the excluded volumes), full `flips`, and the method confound (`checker_before/after`, `schema_before/after`, `method_changed`). 404 when the run has no stored delta — a first sweep is not "nothing changed". |
...
| `src/routes/deltas.rs` | `GET /api/runs/{id}/delta` — the stored two-run comparison, read back. The one legitimate two-run figure; its module doc says why. |
