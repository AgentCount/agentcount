...
| 6 | `settled` | Does the payTo have on-chain settlement history at a pinned block — facilitator-agnostic, with first/last settlement, count, and **distinct payers**? Our own shopper payments (§10.4) are excluded by wallet address. Two limits are stated on every row: the scan covers a **stated block window**, so `fail` means "no settlement in that window" and never "never paid"; and it counts **all** incoming stablecoin transfers, which is a superset of x402 settlements. Distinct payers is the load-bearing figure — the first live scan found one payee taking 162,531 transfers from 29 payers in eleven hours, which a settlement count alone would have published as the busiest seller in the census. |
